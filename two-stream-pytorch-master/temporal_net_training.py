...
parser.add_argument('--resume', default='/home/wangccy/two-stream-pytorch-master/checkpoints', type=str, metavar='PATH',
...
#    train_split_file = os.path.join(args.settings, args.dataset, train_setting_file)
 ...
 #       print("No split file exists in %s directory. Preprocess the dataset first" % (args.settings))
  ...
  #                                                video_transform=val_transform)
    ...
    window = 11
    train_dataset = load_HAA.HAADataset(path,window,usage='Train') 

    val_dataset = load_HAA.HAADataset(path,window,usage='Test')
    ...
    #if True:
    #    checkpoint_name = "%03d_%s" % (0 + 1, "checkpoint.pth.tar")
    #    save_checkpoint({
    #        'epoch': 0 + 1,
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, data in enumerate(train_loader):
        ...
        #print('iter_size is ', args.iter_size)
        # measure accuracy and record loss
        prec1, prec3 = accuracy(output.data, target, topk=(1, 3))
       # print("prec1 is ", prec1)
