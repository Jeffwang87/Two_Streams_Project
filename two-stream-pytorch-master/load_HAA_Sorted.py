...
class HAADataset(Dataset):
    def __init__(self, path, window, transform, usage, ratio=0.8):
        self.path = path
        self.window = window
        ...
        #num_videos = math.ceil(len(videopaths)*ratio)
        num_videos = ratio * 20
        # select train or test
        if usage == 'Train':
            self.videopaths = [x for x in videopaths if int(x[0][-3:]) < num_videos]       
        elif usage == 'Test':
            self.videopaths = [x for x in videopaths if int(x[0][-3:]) >= num_videos]
        else:
